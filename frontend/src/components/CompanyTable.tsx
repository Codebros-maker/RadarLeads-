  ...
  return (
    <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
      <div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between dark:border-slate-700">
        <div>
          <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Empresas</h2>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{companies.length} registros filtrados</p>
        </div>
        <button
          type="button"
          onClick={() => exportToExcel(companies)}
          disabled={companies.length === 0}
          className="inline-flex h-10 items-center justify-center gap-2 rounded-md border border-slate-200 bg-white px-4 text-sm font-semibold text-ink transition hover:border-mint hover:text-mint disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 dark:hover:border-mint dark:hover:text-mint"
        >
        ...
        <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
          <thead className="bg-slate-50 dark:bg-slate-700">
            <tr>
              {['Empresa', 'Categoria', 'Telefone', 'Site', 'Score'].map((header) => (
                <th key={header} className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                  {header}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
            {companies.map((company) => (
              <tr key={company.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-700/50">
                <td className="max-w-xs px-5 py-4">
                  <p className="font-semibold text-ink dark:text-slate-100">{company.name}</p>
                  <p className="mt-1 line-clamp-1 text-xs text-slate-500 dark:text-slate-400">{company.address ?? 'Endereco nao informado'}</p>
                </td>
                <td className="px-5 py-4 text-sm text-slate-600 dark:text-slate-300">{company.category ?? 'Nao informado'}</td>
                ...
                <td className="px-5 py-4 text-sm">
                  {company.website ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-2 font-medium text-mint hover:text-ink dark:hover:text-slate-100"
                    >
                ...
                <td className="px-5 py-4">
                  <span className="inline-flex min-w-14 justify-center rounded bg-ink px-3 py-1 text-sm font-semibold text-white dark:bg-slate-700 dark:text-slate-100">
                    {company.score}
                  </span>
                </td>
              </tr>
